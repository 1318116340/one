<template>
  <el-submenu :index="menu.path || uniqueId">
    ...
    <template v-for="(child, childIndex) in menu.children">
      <d2-layout-header-aside-menu-item v-if="child.children === undefined" :menu="child" :key="childIndex" class="child" />
      <d2-layout-header-aside-menu-sub v-else :menu="child" :key="childIndex" class="child"/>
    </template>
  </el-submenu>
</template>
